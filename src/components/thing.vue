<template lang='pug'>
  .thing(
    ...
    :class=`{
      'inventory-mode': optIn('inventory mode'),
      'icon': optIn('icon'),
      'wide': optIn('wide'),
      'feed': optIn('feed'),
      'comments': optIn('comments'),
      'editing': optIn('editing'),
      'inventory-item': optIn('inventory item'),
      'shallow': optIn('less padding'),
      'debate': optIn('debate', gosmart(thingMutable, 'names', [])),
      'a-lopu-root': optIn('a lopu root', gosmart(thingMutable, 'names', [])),
      'show-options': optIn('show options'),
      'show-value': optIn('show value'),
      'tight-list-thing': optIn('tight list thing'),
      'inventory-slot': optIn('inventory slot'),
      'show-slots': optIn('show slots'),
      'in-slot': optIn('in slot'),
      'shrink-slot-icon': optIn('shrink slot icon'),
      'holo-mode': optIn('holo mode'),
      'a-lopu-map': optIn('a lopu map', gosmart(thingMutable, 'names', [])),
      'put-inventory-first': optIn('put inventory first'),
      'put-properties-first': optIn('put properties first'),
      'put-value-first': optIn('put value first'),
      'put-options-first': optIn('put options first'),
      'put-inventory-last': optIn('put inventory last'),
      'put-properties-last': optIn('put properties last'),
      'put-value-last': optIn('put value last'),
      'put-options-last': optIn('put options last'),
      'property': optIn('property', gosmart(thingMutable, 'names', [])),
      'value': optIn('a lopu value', gosmart(thingMutable, 'names', [])),
      'no-options': optIn('no options')
      }`
    v-if="thingMutable && !optIn('waiting-hidden', gosmart(thingMutable, 'names', [])) && !optIn('deleted', gosmart(thingMutable, 'names', []))"
    :style=`
    getsmart(thingMutable, 'properties.mapped.css.json', undefined)
    `
    )
    // takes too long
    ...
    .main-container
      .main-positioner
        ...
        .option-container.title-container(
          v-show=`
            (
              !optIn('loading', gosmart(thingMutable, 'names', [])) 
              && 
          ...
          `
          @click="showOptionsPerm(); popOpt('icon')"
          )
          .option-positioner.title-positioner(
            ...
            h4.title.option(
              ...
              v-html=`
                thingMutable.title 
                || 
                getsmart(thingMutable, 'names.0', 'Something')
                ||
                (
                  thingMutable.properties 
                  && 
                  thingMutable.properties.mapped 
                  && 
                  thingMutable.properties.mapped['fbData'] 
                  && 
                  thingMutable.properties.mapped['fbData'].story 
                )
                
                `
              title="title"
              )
            //- .debug {{ thingTitleInputOptions }}
            .title.option
              medium-editor(
                v-if=`
                  (
                ...
                v-show=`
                  ...
                  !optIn('loading', gosmart(thingMutable, 'names', []))
                  `
                :options="thingTitleInputOptions"
                custom-tag='h4'
                :text="gosmart(thingMutable, 'title', '')"
                @edit="updateTitle"
                )
            .type-container(
              v-if="optIn('property', this.gosmart(thingMutable, 'names', [])) || optIn('is a typed lopu')"
              )
              .type-positioner
                .type(
                  v-if="!optIn('editing')"
                ...
                v-select.type(
                  ...
                  :pushTags="false"
                  :value=`typeString == 'undefined' ? undefined : typeString`
                  :inputStore=`true`
                  :options="types" 
                  :taggable=`true`
                  placeholder="type"
            ...
            .value-render-container(
              ...
              //- @click="toggleOpt('show value'); showOptionsPerm(); popOpt('icon')"
              .value-render-positioner
                .option.value-render(
                  v-html=`valueShortRender`
                  ) 
        b.vis.name-container(
          v-if=`
        ...
        .option-container.name-container(
          ...
          .option-positioner.name-positioner
            .name.option(v-if="thingMutable.names && thingMutable.names.length > 0 && !optIn('inventory mode')") {{ typeof thingMutable.names[0] == 'object' ? thingMutable.names[0].name : thingMutable.names[0] }}
            .name.option(v-else-if="thingMutable.title" v-html="thingMutable.title")
            .name.option(v-else-if="thingMutable.description" v-html="thingMutable.description")
            .name.option(v-else-if="typeof thingMutable == 'string'") {{ thingMutable }}
            .name.option(v-else) Something
        b.vis.description-container(
          v-if=`
            (
              (
        ...
        .option-container.description-container(
          v-show=`
            (
              (
                !anyOptsIn(['loading', 'property', 'a lopu value'], gosmart(thingMutable, 'names', []))
          ...
          .option-positioner.description-positioner
            p.description.option(
              v-if="!optIn('editing') && !optIn('icon') && !optIn('loading', gosmart(thingMutable, 'names', []))"
              v-html="getsmart(thingMutable, 'description', undefined) ? thingMutable.description : getsmart(thingMutable, 'properties.mapped.fbData.story', undefined) || 'sup?'"
              title="description"
              )
            medium-editor.option(
              v-if="(optIn('edited') || optIn('editing'))"
              v-show="optIn('editing') && !optIn('icon') && !optIn('loading', gosmart(thingMutable, 'names', []))"
              :options="thingDescriptionInputOptions"
              custom-tag='p'
              :text="gosmart(thingMutable, 'description', '')"
        ...
        b.vis.status-container(
          v-if="optIn('loading', gosmart(thingMutable, 'names', []))"
          )
        .option-container.status-container(
          v-show="optIn('loading', gosmart(thingMutable, 'names', []))"
          )
        ...
        b.vis.realms-container(
          v-if="optIn('realms')"
          )
        .option-container.realms-container(
          v-show="optIn('realms')"
          @click="showOptionsPerm(); popOpt('icon')"          
          :class=`{
            vis: optIn('realms')
          ...
          .option-positioner(v-if="optIn('realms')")
            .option.pill.realms
              ...
              v-select(
                ...
                multiple 
                taggable 
                v-model="thingMutable.realms" 
                :options="realms" 
                placeholder="realms"
                )
        //- b.vis.type-container(
          v-if="optIn('type')"
          )
        //- .option-container.type-container(
          v-show="optIn('type')"
          :class=`{
            vis: optIn('type')
          }`
          ...
          .option-positioner(v-if="optIn('type')")
            .option.pill.thingType
              ...
              v-select(
                ...
                v-show="optIn('editing')" 
                :pushTags="true" 
                v-model="thingMutable.types" 
                :options="thingTypes" 
                placeholder="thing type"
                )
                
        b.vis.names-container(
          v-if="optIn('names')"
          )
        .option-container.names-container(
          v-show="optIn('names')"
          :class=`{
            vis: optIn('names')
          }`
          ...
          .option-positioner(v-if="optIn('names')")
            .option.pill.thingNames
              //- .label(v-if="!optIn('editing')") thing
              //- .label(v-else) thing
              ...
              v-select(
                ...
                taggable 
                multiple 
                v-model="thingMutable.names" 
                :returnLabeledValues=`true`
                :options="thingNames" 
                placeholder="thing names"
                )
        b.vis.privacy-container(
          v-if="optIn('privacy')"
          )
        .option-container.privacy-container(
          v-show="optIn('privacy')"
          :class=`{
            vis: optIn('privacy')
          }`
          @click="showOptionsPerm(); popOpt('icon')"          
          )
          .option-positioner(v-if="optIn('privacy')")
            .option.pill.privacy
              //- .label(v-if="!optIn('editing')") thing
              //- .label(v-else) thing
              .value(v-if="!optIn('editing')") {{ thingMutable.thing }}
              ...
              v-select( 
                ...
                taggable 
                multiple 
                v-model="thingMutable.privacy" 
                :options="thingNames" 
                placeholder="these eyes only"
                )

        b.vis.contracts-container(
          v-if="optIn('prices')" 
          )
        .option-container.contracts-container(
          v-show="optIn('prices')" 
          :class=`{
            vis: optIn('prices')
          }`
          ...
          .option-positioner(v-if="optIn('prices')" @click='showOptionsPerm' )
            .option.pill.contract
              template()
                .label-group
                  .label costs
        ...
        b.vis.location-container(
          v-if="anyOptsIn(['show map', 'location'])"
          )
        .option-container.location-container(
          v-show="anyOptsIn(['show map', 'location'])"
          :class=`{
            vis: anyOptsIn(['show map', 'location'])
          }`
          )
          .option-positioner(v-if=`anyOptsIn(['show map', 'location', 'location rendered'])`)
            .option.location
              .map-container
                .map-positioner
                  mapbox(
              ...
              .btn-container(
                :class=`{
                  'courier-job': optIn('courier job', thingMutable.names),
                  'courier-map': optIn('courier map', thingMutable.names),
                }`
              )
                .btn-positioner
                  .wide-btn(
                    v-if="optIn('a lopu map', gosmart(thingMutable, 'names', [])) && !optIn('locationSet')"
                    @click="addNewChildThing"
                    ) 
                    .wide-btn-content(
                      ) Add something here
                  .wide-btn(
                    v-else-if="optIn('a lopu map', gosmart(thingMutable, 'names', [])) && optIn('locationSet')"
                    @click="popOpt('locationSet')"
                    ) 
                    .wide-btn-content(
                      ) Add something else
                  .wide-btn(
                    v-else-if="!optIn('locationSet')"
                    @click="updateThingLocation({loc: 'map'})"
                    ) 
                    .wide-btn-content(
                      ) Set Location
                  .wide-btn(
        ...
        b.vis.postas-container(
          v-if="optIn('postas')"
          )
        .option-container.postas-container(
          v-show="optIn('postas')"
          :class=`{
            vis: optIn('postas')
          }`
          ...
          .option-positioner(v-if="optIn('postas')")
            .option.pill.postas
              ...
              v-select(
                ...
                :taggable="true"
                :pushTags="true"
                v-model="thingMutable.ownerAlias"
                :options="alterEgos"
                :value="thingMutable.ownerAlias"
                )
        b.vis.feedback-container(
          v-if="feedback || getsmart(messages, 'length', 0)"
          )
        .option-container.feedback-container(
          v-show="feedback || getsmart(messages, 'length', 0)"
          :class=`{
            vis: feedback
          }`
          )
          .option-positioner()
            .option.feedback(v-if="feedback")
              .msg {{ feedback.message }}
                ...
                .action(
                  v-if="feedback.type == 'delete'"
                  @click="deleteThing(true)"
                  ) Yes
            .option.feedback-list(v-if="messages")
              .feedback-list-container
                .feedback-list-positioner
                  .feedback-list.option-messages.inventory-messages
                    template(
        ...
        b.vis.inventory-container(
          v-if=`(optIn('show inventory') && !optIn('icon')) || optIn('feed')`
          )
        .option-container.inventory-container(
          v-show=`(optIn('show inventory') && !optIn('icon')) || optIn('feed')`
          :class=`{
            vis: (optIn('show inventory') && !optIn('icon')) || optIn('feed')
          }`
          ref="inventory-option"
          @click="showOptionsPerm(); popOpt('icon')"
          )
          .option-positioner
            .option.inventory
              //- .option-title-container.inventory-title-container(
                v-show="!(optIn('inventory mode') || optIn('inventory', gosmart(thingMutable, 'names', [])))"
                )
                .option-title-positioner.inventory-title-positioner
                  h4.option-title.inventory-title Inventory
              .thing-list-container.inventory-list-container(
                ref="inventory-list-container"
                )
                .thing-list.inventory-list(
                  :class=`{
                    'show-slots': optIn('show slots')
                  }`
                  :style=`{
                    width: invGrid.invWidth ? invGrid.invWidth+"px" : undefined
                  }`
                  ref="inventory-list"
                  )
                  template(
                    v-for="(thing, index) in getsmart(thingMutable, 'things', [])"
                    )
                    b(
                    v-if=`
                      index < (
                        (
                          limitMutable == getsmart(thingMutable, 'things.length', 5)+1
                        ) 
                        ? limitMutable 
                        : limitMutable-1
                      )
                    ...
                    .slot-container.inventory-slot-container(
                      :style=`{
                        width: (optIn('icon', getsmart(thing, 'properties.mapped.options.Array of Strings', false)) && optIn('slotted', getsmart(thingMutable, 'properties.mapped.options.Array of Strings', false))) ? invGrid.width+"px" : 'auto',
                        height: (optIn('icon', getsmart(thing, 'properties.mapped.options.Array of Strings', false)) && optIn('slotted', getsmart(thingMutable, 'properties.mapped.options.Array of Strings', false))) ? invGrid.width+"px" : 'auto',
                        padding: optIn('show slots') ? invGrid.padding : undefined
                      }`
                      v-show="index < ((limitMutable == getsmart(thingMutable, 'things.length', 5)+1) ? limitMutable : limitMutable-1)"
                      v-if=`optIn('slotted')`
                      )
                      .slot-positioner(
                        :style=`{
                        ...
                        thing(
                          v-if=`
                            thing.title !== 'debug'
                            &&
                            anyOptsIn(['keep inventory loaded', 'show inventory'])
                            &&
                            !optIn('dont load children') 
                            && 
                            thing
                            &&
                            !optIn('inventory slot', thing.names)
                            `
                          :thing="thing"
                          :index="index"
                          :key="thing._id || thing.uuid || thing"
                          :parentId="thingMutable._id || thingMutable.uuid || _uid"
                          :schema=`getsmart(thingMutable, 'properties.mapped.things schema', undefined)`
                          :parentOptions=`childOptions`
                          :style=`{
                            order: ( optIn('put things first') && !optIn('new', thing.names) ) ? 2
                            : ( optIn('put things last') && !optIn('new', thing.names) ) ? 4
                            : ( optIn('put new things first') && optIn('new', thing.names) ) ? 2
                            : ( optIn('put new things last') && optIn('new', thing.names) ) ? 4
                            : 3
                            }`
                          @remove="deleteChildThing(index, 'things')"
                          @saved="savedChildThing(index)"
                          )
                          // @invlist
                    thing(
                      v-else-if=`
                  ...
                  .slot-container.inventory-slot-container(
                    v-if=`optIn('slotted')`
                    v-show=`!optIn('not-addable') && anyOptsIn(['addable with template', 'template adder'])`
                    :class=`{
                      vis: !optIn('not-addable') && anyOptsIn(['addable with template', 'template adder'])
                    }`
                    :style=`{
                      width: (invGrid.width && optIn('slotted')) ? invGrid.width+"px" : 'auto' ,
                      height: (invGrid.width && optIn('slotted')) ? invGrid.width+"px" : 'auto' ,
                      padding: optIn('show slots') ? invGrid.padding : undefined,
                    }`
                    )
                    .slot-positioner
                      thing.in-slot(
                        v-if=`!optIn('not-addable') && anyOptsIn(['addable with template', 'template adder'])`
                        :thing=`defaultThingMutable`
                        :savesTo="savesToMutable"
                        @saved="resetDefaultThing()"
                        )
                  thing(
                    v-else-if=`!optsIn(['not-addable', 'slotted']) && anyOptsIn(['addable with template', 'template adder'])`
                    v-show=`!optIn('not-addable') && anyOptsIn(['addable with template', 'template adder'])`
                    :thing=`defaultThingMutable`
                    :savesTo="savesToMutable"
                    @saved="resetDefaultThing()"
                  ...
                  .slot-container.inventory-slot-container(
                    v-if=`optIn('slotted')`
                    v-show=`!anyOptsIn(['not-addable', 'no adder button', 'hide addable button'])`
                    :class=`{
                    'put-addable-first': optIn('put addable first'),
                    'put-addable-last': optIn('put addable last'),
                    'wide': optIn('wide-adder') || optIn('feed'),
                    rounded: optIn('rounded'),
                    vis: !anyOptsIn(['not-addable', 'no adder button', 'hide addable button'])
                    }`
                    :style=`{
                      width: (invGrid.width && optIn('slotted')) ? invGrid.width+"px" : 'auto' ,
                      height: (invGrid.width && optIn('slotted')) ? invGrid.width+"px" : 'auto' ,
                    ...
                    .slot-positioner
                      .thing.meta-thing.add-new.in-slot(
                        @click="addNewChildThing"
                        :class=`{
                          'shrink-slot-icon': optIn('shrink slot icons'),
                          'icon': !optIn('wide adder')
                        }`
                        )
                        .meta-thing-square
                          icon(name="plus")
                  .thing.meta-thing.add-new(
                    v-else-if=`!optIn('slotted')`
                    v-show=`!anyOptsIn(['not-addable', 'no adder button', 'hide addable button'])`
                    @click="addNewChildThing"
                    :class=`{
                      'put-addable-first': optIn('put addable first'),
                  ...
                  .slot-container(
                    v-if=`optIn('slotted')`
                    :class=`{
                      'put-addable-first': optIn('put-thing-loader-first'),
                      'put-addable-last': optIn('put-thing-loader-last'),
                      'wide': (optIn('wide') || optIn('feed')) && !optIn('normal-loader'),
                      'rounded': optIn('rounded'),
                      'vis': (thingMutable.things && getsmart(thingMutable, 'things.length', 5) > 0 && limitMutable < getsmart(thingMutable, 'things.length', 5))
                      }`
                    v-show="getsmart(gosmart(thingMutable, 'things', []), 'length', 0) > limitMutable"
                    :style=`{
                      width: (invGrid.width && optIn('slotted')) ? invGrid.width+"px" : 'auto' ,
                      height: (invGrid.width && optIn('slotted')) ? invGrid.width+"px" : 'auto' ,
                      padding: optIn('show slots') ? invGrid.padding : undefined,
                    }`
                    @click="limitMutable = getsmart(thingMutable, 'things.length', 5)+1"
                    )
                    .slot-positioner
                      .thing.meta-thing.load-next.in-slot(
                        :class=`{
                          'shrink-slot-icon': optIn('shrink slot icons'),
                          'icon': !optIn('wide adder')
                        }`
                        ...
                        .meta-thing-square
                          .name Load {{ getsmart(thingMutable, 'things.length', 5) - limitMutable  }} more {{ thingMutable.title ? thingMutable.title.replace(/.*-/, '') : '' }}
                  .thing.meta-thing.load-next(
                    v-else-if=`!optIn('slotted')`
                    v-show="getsmart(gosmart(thingMutable, 'things', []), 'length', 0) > limitMutable"
                    :class=`{
                      'put-addable-first': optIn('put-thing-loader-first'),
              ...
              .option-messages.inventory-messages
        ...
        //- b.vis.properties-container(
          v-if="(optIn('show properties') && !optIn('icon'))"
          )
        //- .option-container.properties-container.indented-container(
          v-show="(optIn('show properties') && !optIn('icon'))"
          :class=`{
            vis: (optIn('show properties') && !optIn('icon'))
          }`
          @click="showOptionsPerm(); popOpt('icon')"
          )
          .option-positioner
            .option.properties
              .option-title-container.properties-title-container
                .option-title-positioner.properties-title-positioner
                  h4.option-title.properties-title Properties
                  
              .thing-list-container.properties-list-container.tight-thing-list-container
                .thing-list.properties-list(ref="properties")
                  template(
                    v-for="(thing, index) in thingMutable.properties"
                    )
                    b(
                    v-if="index < propertiesLimitMutable && !optIn('dont load children') && (optIn('show properties') && !optIn('icon'))"
                    )
                    thing(
                      v-if=`
                        index < propertiesLimitMutable 
                        && 
                        !optIn('dont load children') 
                        && 
                        (
                          optIn('show properties') 
                          && 
                          !optIn('icon')
                        )
                        `
                      :thing="thing"
                      :index="index"
                      :key="thing ? thing._id || thing : thing"
                      :parentId="thingMutable._id || thingMutable.uuid || _uid"
                      :parentOptions=`[
                        optIn('addable') ? 'addable' : null,
                        optIn('feed') ? null : 'properties-item',
                        'property',
                        optIn('icon') ? 'no-fetch' : null,
                        optIn('dont load childrens children') ? 'dont load children' : null,
                        optIn('dont load children children children') ? 'dont load childrens children' : null,
                        optIn('show-children-comments') ? 'show-comments' : null,
                        optIn('shallow adders') ? 'shallow adders' : null,
                        firstOrLast(index, propertiesLimitMutable, thingMutable.properties, 'thing') || null,
                        ]`
                      :style=`{
                        }`
                      @remove="deleteChildThing(index, 'properties')"
                      )
                      //- order: ( optIn('put-properties-first') && !optIn('new', thing.names) ) ? 2
                      //- : ( optIn('put-properties-last') && !optIn('new', thing.names) ) ? 4
                      //- : ( optIn('put-new-properties-first') && optIn('new', thing.names) ) ? 2
                      //- : ( optIn('put-new-properties-last') && optIn('new', thing.names) ) ? 4
                      //- : 3 
                      // @properties
                  //- template(
                    v-for="(thing, key, index) in thingMutable"
                    )
                    .debug {{ thing }}
                    .debug {{ key }}
                    .debug {{ index }}
                    ...
                    //- thing(
                      ...
                      //- : 3 
                      // @properties
                  .slot-container.property-slot-container(
                    v-show="optIn('dont-load-properties') || (thingMutable.properties && propertiesLimitMutable < thingMutable.properties.length)"
                    )
                    .slot-positioner                  
                      .thing.meta-thing.load-next(
                        :class=`{
                        ...
                        :style=`{ order: 5 }`
                        @click='propertiesLimitMutable = thingMutable.properties.length'
                        )
                        .meta-thing-square
                          .name Load {{ true ? '' : thingMutable.properties ? propertiesLimitMutable < thingMutable.properties.length ? propertiesLoadLimitMutable < thingMutable.properties.length - propertiesLimitMutable ? propertiesLoadLimitMutable + ' ' : thingMutable.properties.length - propertiesLimitMutable + ' ' : thingMutable.properties.length  + ' ' : '' }}more {{ thingMutable.title ? thingMutable.title.replace(/.*-/, '') : '' }}

                  //- .slot-container.property-slot-container
                    .slot-positioner                
                      thing(
                        v-show=`optIn('addable') && optIn('addable with template')`
                        :thing=`{
                          names: ['new'],
                          properties: [
                            {
                              ...
                              list: [
                                'editing',
                                'edited',
                                optIn('shallow adders') ? 'less padding' : null
                              ]
                            }
                          ],
                          owners: [entity._id],
                  ...
                  .slot-container.property-slot-container(
                    v-show="!optIn('properties not addable') && !optIn('addable with template')"
                    )
                    .slot-positioner                
                      .thing.meta-thing.add-new(
                        :class=`{
              ...
              .option-messages.properties-messages
        ...
        //- b.vis.value-container(
          v-if=`
            (
              optIn('a lopu value', gosmart(thingMutable, 'names', []))
              || 
              optIn('show value') 
            )
            && 
            !optIn('icon')
            &&
            getsmart(thingMutable, 'type', undefined)
            &&
            getsmart(thingMutable, 'type', undefined) !== 'Thing'
            `
          )
        //- .option-container.value-container.indented-container(
          v-show=`
            (
              optIn('a lopu value', gosmart(thingMutable, 'names', []))
              || 
          ...
          .option-positioner(
            ...
            .option.value(
              v-if=`                
                ...
                && 
                (
                  typeof getsmart(thingMutable, 'type', false) == 'string'
                  &&
                  (
                    getsmart(thingMutable, 'type', '').indexOf('String') == 0 
                    || 
                    getsmart(thingMutable, 'type', '').indexOf('Number') == 0 
                    || 
                    getsmart(thingMutable, 'type', '').indexOf('Boolean') == 0
                  )
                )
              `
            ) {{ valueShortRender }}
            medium-editor.option.value(
              v-if=`
                (
                  optIn('edited') 
                  || 
              ...
              v-show=`
                ...
                !optIn('loading', gosmart(thingMutable, 'names', []))
              `
              :options="thingValueInputOptions"
              custom-tag='p'
              :text="valueString"
              @edit="updateValue"
              :class=`{
            ...
            .option.value(
              v-show=`
                ...
                &&
                (
                  getsmart(thingMutable, 'type', '').indexOf('Array') == 0
                  ||
                  getsmart(thingMutable, thingMutable.type, {}).constructor == Array
                )
                `
              )
              .thing-list-container.value-list-container.tight-thing-list-container(
                ref="values"
                )
                .thing-list
                  template(
                    v-for="(thing, index) in getsmart(thingMutable, thingMutable.type, [])"
                    v-if=`
                      typeof getsmart(thingMutable, 'type', false) == 'string'
                      &&
                      (
                    ...
                    thing(
                      v-if=`typeof thing !== 'string'`
                      v-show=`
                        optIn('property', gosmart(thingMutable, 'names', []))
                        `
                      :thing="valueToAThing(thing)"
                      :index="index"
                      :key="thing ? thing._id || thing : thing"
                      :parentId="thingMutable._id || thingMutable.uuid || _uid"
                      :parentOptions=`[
                        optIn('addable') ? 'addable' : null,
                        optIn('feed') ? null : 'value-item',
                        optIn('icon') ? 'no-fetch' : null,
                        optIn('dont load childrens children') ? 'dont load children' : null,
                        optIn('dont load children children children') ? 'dont load childrens children' : null,
                        optIn('show-children-comments') ? 'show-comments' : null,
                      ...
                      @remove="deleteChildThing(index, thingMutable.type)"
                      )
                    medium-editor.option.value(
                      v-if=`
                        (
                      ...
                      :text="thing"
                      @edit=`
                        (op)=>getsmart(thingMutable, thingMutable.type, [])[index] = op.api.origElements.innerHTML`
                      :class=`{
                        'value': true,
                        'value-input': true
                      }`
                      ...
                      // @valist @vallist
                    .button-container(
                      @click=`deleteChildThing(index, thingMutable.type)`
                      v-if=`
                        (
                          optIn('edited') 
                          || 
                      ...
                      .button-positioner
                        .button Delete
                    //- value(
                    //-   v-if="!optIn(getsmart(thingMutable, 'type', ''), ['Array of Arrays', 'Array of Things', 'Array'])"
                    //-   :value=`thing`
                    //-   :key=`thing ? thing._id || thing : thing`
                    //-   )
                      //- order: ( optIn('put-value-first') && !optIn('new', thing.names) ) ? 2
                      //- : ( optIn('put-value-last') && !optIn('new', thing.names) ) ? 4
                      //- : ( optIn('put-new-value-first') && optIn('new', thing.names) ) ? 2
                      //- : ( optIn('put-new-value-last') && optIn('new', thing.names) ) ? 4
                      //- : 3 
                  .slot-container
                    .slot-positioner
                      .thing.meta-thing.load-next(
                        ...
                        v-if="optIn('dont-load-value') || (thingMutable.value && valueLimitMutable < thingMutable.value.length)"
                        :style=`{ order: 5 }`
                        @click='valueLimitMutable = gosmart(thingMutable, thingMutable.type, []).constructor == Array ? thingMutable[thingMutable.type].length : 5'
                        )
                        .meta-thing-square
                          .name Load {{ true ? '' : thingMutable.value ? valueLimitMutable < thingMutable.value.length ? valueLoadLimitMutable < thingMutable.value.length - valueLimitMutable ? valueLoadLimitMutable + ' ' : thingMutable.value.length - valueLimitMutable + ' ' : thingMutable.value.length  + ' ' : '' }}more {{ thingMutable.title ? thingMutable.title.replace(/.*-/, '') : '' }}
                  .slot-container
                    .slot-positioner
                      .thing.meta-thing.add-new(
                        :class=`{
                        ...
                        v-if="!optIn('addable') && !optIn('addable with template')"
                        :style=`{
                          order: 4
                        }`
                        @click="addNewValue"
            ...
            .option.value(
              ...
              &&
              (
                getsmart(thingMutable, 'type', '').indexOf('Thing') == 0
                ||
                getsmart(thingMutable, 'type', '').indexOf('json') == 0
              )
              `
              )
              thing(
                :thing=`getsmart(thingMutable, getsmart(thingMutable, 'type', 'Thing'), false)`
              )
        b.vis.comments-container(
          v-if="(optIn('show-comments') && !optIn('icon'))"
          )
        .option-container.comments-container(
          v-show="(optIn('show-comments') && !optIn('icon'))"
          :class=`{
            vis: (optIn('show-comments') && !optIn('icon'))
          }`
          )
          .option-positioner.comments-sizer
            .option.comments
              thing(
                v-if="!optIn('dont-load-comments') && getsmart(thingMutable, 'things.mapped.all-comments', false)"
                :thing="getsmart(thingMutable, 'things.mapped.all-comments', undefined)"
                :index="index"
                :key="getsmart(thingMutable, 'things.mapped.all-comments', undefined)"
                :parentOptions=`[
                  'no options',
                  'no context',
                  'feed',
                  ...
                  'shallow adders',
                  'dont load children children children'
                  ]`
                :style=`{
                  order: ( optIn('put things first') && !optIn('new', thing.names) ) ? 2
                  : ( optIn('put things last') && !optIn('new', thing.names) ) ? 4
                ...
                :savesTo=`
                [
                  gosmart(thingMutable, 'things.mapped.this lopus comments', 
                    {
                      parents: [thingMutable],
        ...
        .option-container.icons-container(
          v-show=`
            optIn('show options')
            &&
            !optIn('loading', getsmart(thingMutable, 'names', []))
          ...
          .option-positioner
            .option.delete(
              title="delete"
              v-if=`optIn('show options icons')`
              )
              .label.action(
                @click="deleteThing(false)"
                )
                icon(name="trash")
            //- .option.postas(
              v-if=`
                optsIn(['editing', 'show options icons'])
                `
              title="post as"
              )
              .action(
            ...
            .option.replies(
              ...
              .action(
                @click="toggleOpt('show-comments'); loadComments();"
                )
                icon(name="comments")
            .option.fork(
              v-if=`
            ...
            .option.showinv(
              ...
              .action(
                @click="toggleOpt('show inventory'); popOpt('dont load children');"
                )
                icon(name="cubes")
            .option.showproperties(
              ...
              .action(
                @click="toggleOpt('show properties')"
                )
                icon(name="flask")
            .option.show-value(
              v-if=`
                anyOptsIn(['property','a lopu value'], gosmart(thingMutable, 'names', []))
                &&
                optIn('show options icons')
                `
              title="value"
            ...
            .option.title(
              ...
              .action(
                @click="toggleOpt('show title'); popOpt('hide title');"
                )
                icon(name="envira")
            .option.description(
              v-if=`
              ...
              .action(
                @click="toggleOpt('show description'); popOpt('hide description');"
                )
                icon(name="book")
            .option.statement(
              v-if=`
                ...
                !optIn('editing') 
                && 
                (thingMutable.realms && thingMutable.realms.indexOf('statement') >= 0)
                `
              title="vote"
              )
              .action(
            ...
            .option.map(
              ...
              .label.action(
                @click="toggleOpt('location'); pushOpt('location rendered')"
                )
                icon(
                  name="map-o"
                  )
            ...
            .option.edit(
              ...
              .label.action(
                @click="toggleOpt('editing'); pushOpt('edited')"
                )
                icon(
                  v-if="!optIn('editing')"
                  name="envelope"
            ...
            .option.meta(
              ...
              .action(
                @click="toggleOpt('show options icons')"
                )
                <icon name="cog"></icon>
        ...
        // maybe
          ...
          .option.question(v-if="(!optIn('editing') && thingMutable.realms && (thingMutable.realms.indexOf('question') >= 0) && thingMutable.realms.indexOf('boolean') >= 0)")
            .action.tf true
            .action.tf false
          .option.answer(v-if="(!optIn('editing') && thingMutable.realms && (thingMutable.realms.indexOf('answer') >= 0))")
            .action.tf agree
            .action.tf disagree
          .option.answer(v-if="(!optIn('editing') && thingMutable.realms && (thingMutable.realms.indexOf('answer') >= 0) && thingMutable.realms.indexOf('boolean') >= 0)")
              .action.tf true
              .action.tf false
  ...
  methods: {
         ...
         */
          if(this.thingMutable && typeof this.thingMutable === 'object'){
            // check if it has a uuid
              if(!this.thingMutable.uuid && this.thingMutable.uuid !== false){
                this.thingMutable.uuid = this.$uuid.v4()
              }
            this.reinit()
    ...
    // location
      ...
      autoCompleteAddress(text){
        return new Promise((resolve, reject)=>{
          let url = "https://api.mapbox.com/geocoding/v5/mapbox.places/"+text+".json?access_token=" + mapboxgl.accessToken +"&types=address"
          this.$http.get(url)
          .then((data)=>{
            resolve(data.body.features[0])            
          })
